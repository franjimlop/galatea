...
const Complementarias = () => {
    ...
    useEffect(() => {
        const obtenerNoticias = async () => {
            try {
                const response = await fetch('https://localhost/noticias');
                if (response.ok) {
        ...
        <div className="fondo">
            {!mostrarContenidoCompleto && (
                <div className="container text-center">
                    <h2 className="py-5 color-titulo">Actividades Complementarias</h2>
                    <div className="row pb-3">
                        {/* 4 cards en una línea, por debajo de lg de 2 en 2 */}
                        {noticias.filter(noticia => noticia.categoria === 'complementaria') // Filtrar por categoría
